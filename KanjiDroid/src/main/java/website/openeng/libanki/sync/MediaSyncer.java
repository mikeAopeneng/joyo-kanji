...
package website.openeng.libanki.sync;
 ...
 * made through KanjiDroid (e.g., multimedia note editor, media check) are recorded directly in
            ...
            while (true) {
                ...
                try {
                    ...
                    mCon.publishProgress(String.format(
                            KanjiDroidApp.getAppResources().getString(R.string.sync_media_changes_count), toSend));
    ...
    private void _downloadFiles(List<String> fnames) throws APIVersionException {
        ...
        while (fnames.size() > 0) {
            try {
                ...
                mCon.publishProgress(String.format(
                        KanjiDroidApp.getAppResources().getString(R.string.sync_media_downloaded_count), mDownloadCount));
            } catch (IOException e) {
