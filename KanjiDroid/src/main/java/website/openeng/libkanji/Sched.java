...
public class Sched {
        ...
        while (!mNewDids.isEmpty()) {
            ...
            if (lim != 0) {
                ...
                if (!mNewQueue.isEmpty()) {
                    // Note: libkanji reverses mNewQueue and returns the last element in _getNewCard().
                    // KanjiDroid differs by leaving the queue intact and returning the *first* element
                    // in _getNewCard().
                    return true;
                }
            }
                ...
                if (!mRevQueue.isEmpty()) {
                    ...
                    try {
                        if (mCol.getDecks().get(did).getInt("dyn") != 0) {
                            ...
                            // Note: libkanji reverses mRevQueue and returns the last element in _getRevCard().
                            // KanjiDroid differs by leaving the queue intact and returning the *first* element
                            // in _getRevCard().
                        } else {
                            Random r = new Random();
    ...
    private int _fuzzedIvl(int ivl) {
        ...
        // returns x in [0, b-a), hence the +1 diff with libkanji
    ...
    /**
     * Sorts a card into the lrn queue LIBANKI: not in libkanji
     */
