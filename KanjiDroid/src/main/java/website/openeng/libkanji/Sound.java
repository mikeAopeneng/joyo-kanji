...
/**
 * Class used to parse, load and play sound files on KanjiDroid.
 */
public class Sound {
...
//                    Integer.parseInt(soundPath.substring(3, 4)));
            ...
            try {
                ...
                if (sAudioManager == null) {
                    sAudioManager = (AudioManager) KanjiDroidApp.getInstance().getApplicationContext().getSystemService(Context.AUDIO_SERVICE);
                }
                ...
                sMediaPlayer.setDataSource(KanjiDroidApp.getInstance().getApplicationContext(), soundUri);
    ...
    private static void configureVideo(VideoView videoView, int videoWidth, int videoHeight) {
        // get the display
        Context context = KanjiDroidApp.getInstance().getApplicationContext();
